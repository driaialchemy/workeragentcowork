...
workeragentcowork is a Python-based automated research pipeline focused on Claude Cowork and Claude Code cost optimization. It fetches articles from RSS feeds and web pages, summarizes them using OpenAI, cross-verifies factual claims against independent sources, and produces a plain-text cost optimization briefing report. It solves the problem of manual research aggregation by automating the workflow from source collection to final report delivery via email. The system is not agentic in the tool-calling sense - workers execute in a fixed, predetermined order.

## 2. How It Works (End-to-End Flow)

When you run `python orchestrator.py "Claude Cowork and Claude Code cost optimization"`, the system executes this sequence:
...
- Manual dispatch with optional topic input (default: "Claude Cowork and Claude Code cost optimization")
