...
def extract_claims(summaries, topic):
    ...
    try:
        ...
        if not exc.recoverable:
            raise
        return _fallback_claims(summaries)
...
def _fallback_claims(summaries):
    """Extract simple sentence-level claims when OpenAI is temporarily unavailable."""
    claims = []
    for idx, summary in enumerate(summaries):
        text = summary.get("summary", "")
        sentences = [s.strip() for s in re.split(r"(?<=[.!?])\s+", text) if s.strip()]
        for sentence in sentences[:1]:
            claims.append((sentence[:500], idx))
        if len(claims) >= 5:
            break
    return claims
...
def verify_claim(claim, source_idx, summaries):
    ...
    try:
        ...
        return (
            "Unverifiable",
            "OpenAI verification was temporarily unavailable; this claim was not independently verified.",
        )
