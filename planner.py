...
def create_plan(topic):
    ...
    try:
        ...
        if not exc.recoverable:
            raise
        print(f"  Warning: {exc}")
        print("  Using a local fallback research plan so the run can continue.")
...
def _fallback_plan(topic):
    """Create a deterministic plan when the AI planner is temporarily unavailable."""
    focus_areas = [
        f"Cost drivers and usage patterns related to {topic}",
        f"Token, context, tool-call, and orchestration waste relevant to {topic}",
        f"Governance, scoping, and workflow controls for {topic}",
        f"Practical implications for CFOs, AI consultants, DevOps teams, managers, and knowledge workers",
    ]
    steps = [
        "Collect articles from sources.txt",
        "Evaluate evidence coverage",
...
def evaluate_evidence(topic, focus_areas, articles):
    ...
    try:
        ...
        if not exc.recoverable:
            raise
        return _fallback_evidence_check(focus_areas, articles)
...
def _fallback_evidence_check(focus_areas, articles):
    """Approximate coverage with keyword matching when OpenAI is unreachable."""
    corpus = " ".join(
        f"{article.get('title', '')} {article.get('content', '')}" for article in articles
    ).lower()
    missing = []
    for area in focus_areas:
        words = [w.strip(".,:;!?()[]").lower() for w in area.split() if len(w) > 4]
        if words and not any(word in corpus for word in words):
            missing.append(area)
    if missing:
        return False, "OpenAI coverage evaluation was unavailable; local keyword coverage found gaps.", missing
    return True, "OpenAI coverage evaluation was unavailable; local keyword coverage found no obvious gaps.", []
