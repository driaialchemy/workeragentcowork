...
from utils.ai_client import AIServiceError
...
def check_environment():
    missing = [
        ...
        if not os.environ.get(v) or os.environ.get(v, "").strip() in {"sk-...", "your-key-here"}
    ]
    if not missing:
        return
    ...
    print("       SMTP_USER=")
    print("       Set SMTP_APP_PASSWORD in .env if email delivery is enabled")
    print("       EMAIL_TO=")
    print()
...
def run(topic, email_to=None, verbose=True):
    ...
    if not topic:
        print("  Error: topic cannot be blank.")
        return None

    print()
    print("=" * 60)
    print("  WORKERAGENTCOWORK BRIEFING SYSTEM")
    print("=" * 60)
    print(f"\n  Topic: {topic}\n")
    ...
    if recipient:
        ...
        success, msg = send_report(
            ...
            subject=f"Claude Cost Optimization Briefing: {topic}",
            body=report,
        )
        if success:
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        description="workeragentcowork - planner-worker briefing system for Claude Cowork and Claude Code cost optimization."
    )
    ...
    result = run(args.topic, email_to=args.email, verbose=not args.quiet)
